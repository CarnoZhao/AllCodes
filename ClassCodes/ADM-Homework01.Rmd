...
After testing the function several times, I found that the parameters $\epsilon = 0.05$ and $\alpha_n = \frac{1}{n}$ are not proper to find the minimum of $f(x)$. First, the $\epsilon$ is too large both for $x$ value and $f(x)$ value based on their scales, which makes the function repeat less times, then stop at a wrong point. Second, the step size $\alpha_n$ decreases too fast, making $|x_{n + 1} - x_n|$ reach $\epsilon$ quickly.
...
```{r, out.width = '30em', fig.align = 'center', echo = FALSE}
xs = runif(100, -2, 4)
gred.des2 = function(x1, epsilon = 0.001, which.n = '1/sqrt(n)'){
	n = 1
	x.data = c(x1)
	while(n == 1 || abs(x1 - pre) > epsilon){
		pre = x1
