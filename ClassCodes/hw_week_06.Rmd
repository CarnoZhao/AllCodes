...
When height is centered, $E(\text{height})$ becomes $0$, so $E(a) = E(age)$.
...
When `height` is standardized, $Var(\text{height})$ becomes $1$. So does the standard deviation of height. So $Var(b)$ changes. In addition, $Var(a)$ comes from the modeling, which makes $Var(a)$ much smaller than $Var(\text{age})$.

Thus, $Var(\text{height}) = 1\text{ and }Var(a) \ll Var(\text{age})$.

Then, $Var(b) \approx Var(\text{age})$.
